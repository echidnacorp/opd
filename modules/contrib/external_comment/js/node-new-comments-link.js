...
(function ($, Drupal, drupalSettings) {
  ...
  function processNodeNewCommentLinks($placeholders) {
    ...
    var fieldName = 'external_comment';
    var $placeholder;
      ...
      for (var nodeID in results) {
        if (results.hasOwnProperty(nodeID) && $placeholdersToUpdate.hasOwnProperty(nodeID)) {
          $placeholdersToUpdate[nodeID]
            .attr('href', results[nodeID].first_new_external_comment_link)
            .text(Drupal.formatPlural(results[nodeID].new_external_comment_count, '1 new comment', '@count new comments'))
            .removeClass('hidden');
    ...
    if (drupalSettings.comment && drupalSettings.comment.newCommentsLinks) {
      ...
      $.ajax({
        url: Drupal.url('external/comments/render_new_comments_node_links'),
        type: 'POST',
