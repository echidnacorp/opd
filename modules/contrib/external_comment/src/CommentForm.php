...
class CommentForm extends ContentEntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['#id'] = Html::getUniqueId('external_comment_form');
    $form['#theme'] = ['external_comment_form__' . $entity->getEntityTypeId() . '__' . $entity->bundle() . '__' . $field_name, 'external_comment_form'];

    $anonymous_contact = $field_definition->getSetting('anonymous');
    $is_admin = $comment->id() && $this->currentUser->hasPermission('administer external comments');

    if (!$this->currentUser->isAuthenticated() && $anonymous_contact != EXTERNAL_COMMENT_ANONYMOUS_MAYNOT_CONTACT) {
    ...
    if (!$comment->id() && !$comment->hasParentComment()) {
      $form['#action'] = $this->url('external_comment.reply', ['entity_type' => $entity->getEntityTypeId(), 'entity' => $entity->id(), 'field_name' => $field_name]);
    }

    $external_comment_preview = $form_state->get('external_comment_preview');
    if (isset($external_comment_preview)) {
      $form += $external_comment_preview;
    }
    ...
    if ($is_admin) {
      ...
      if (empty($external_comment_preview)) {
      ...
      $status = ($this->currentUser->hasPermission('skip external comment approval') ? CommentInterface::PUBLISHED : CommentInterface::NOT_PUBLISHED);
    }
    ...
    // 'administer external comments' is editing an existing comment from an
    // authenticated user.
    ...
    // when a user with the permission 'administer external comments' is editing an
    ...
    $form['author']['name'] = [
      ...
      '#required' => ($this->currentUser->isAnonymous() && $anonymous_contact == EXTERNAL_COMMENT_ANONYMOUS_MUST_CONTACT),
      '#maxlength' => 60,
    ...
    $form['author']['mail'] = [
      ...
      '#access' => ($comment->getOwner()->isAnonymous() && $is_admin) || ($this->currentUser->isAnonymous() && $anonymous_contact != EXTERNAL_COMMENT_ANONYMOUS_MAYNOT_CONTACT),
    ];
    ...
    $form['author']['homepage'] = [
      ...
      '#maxlength' => 255,
      '#size' => 30,
      '#access' => $is_admin || ($this->currentUser->isAnonymous() && $anonymous_contact != EXTERNAL_COMMENT_ANONYMOUS_MAYNOT_CONTACT),
    ];
 ...
 /**
  ...
  protected function actions(array $form, FormStateInterface $form_state) {
    ...
    $element['submit']['#access'] = ($comment->id() && $this->currentUser->hasPermission('administer external comments')) || $preview_mode != DRUPAL_REQUIRED || $form_state->get('external_comment_preview');
  ...
  public function buildEntity(array $form, FormStateInterface $form_state) {
    ...
    if ($comment->id() && $this->currentUser->hasPermission('administer external comments')) {
    ...
    if (trim($comment->getSubject()) == '') {
      if ($comment->hasField('comment_body')) {
        ...
        $external_comment_text = $comment->comment_body->processed;
        $comment->setSubject(Unicode::truncate(trim(Html::decodeEntities(strip_tags($external_comment_text))), 29, TRUE, TRUE));
      }
  ...
  public function preview(array &$form, FormStateInterface $form_state) {
    $external_comment_preview = external_comment_preview($this->entity, $form_state);
    $external_comment_preview['#title'] = $this->t('Preview comment');
    $form_state->set('external_comment_preview', $external_comment_preview);
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($this->currentUser->hasPermission('post external comments') && ($this->currentUser->hasPermission('administer external comments') || $entity->{$field_name}->status == CommentItemInterface::OPEN)) {
      ...
      $page = $this->entityManager->getStorage('external_comment')->getDisplayOrdinal($comment, $field_definition->getSetting('default_mode'), $field_definition->getSetting('per_page'));
      if ($page > 0) {
        $query['page'] = $page;
      }
