...
class CommentLazyBuilders {
   ...
   *   The renderer service.
   */
  public function __construct(EntityManagerInterface $entity_manager, EntityFormBuilderInterface $entity_form_builder, AccountInterface $current_user, CommentManagerInterface $external_comment_manager, ModuleHandlerInterface $module_handler, RendererInterface $renderer) {
   ...
   * @param string $external_comment_type_id
   *   The comment type ID.
   *
   * @return array
   *   A renderable array containing the comment form.
   */
  public function renderForm($commented_entity_type_id, $commented_entity_id, $field_name, $external_comment_type_id) {
    $values = [
      'entity_type' => $commented_entity_type_id,
      'commented_' . $commented_entity_type_id => $commented_entity_id,
      'field_name' => $field_name,
      'external_comment_type' => $external_comment_type_id,
      'pid' => NULL,
    ];
    $comment = $this->entityManager->getStorage('external_comment')->create($values);
   ...
   *
   * @param string $external_comment_entity_id
   ...
   */
  public function renderLinks($external_comment_entity_id, $view_mode, $langcode, $is_in_preview) {
    $links = [
    ...
    if (!$is_in_preview) {
      ...
      $entity = $this->entityManager->getStorage('external_comment')->load($external_comment_entity_id);
      $commented_entity = $entity->getCommentedEntity();

      $links['external_comment'] = $this->buildLinks($entity, $commented_entity);
      ...
      $this->moduleHandler->alter('external_comment_links', $links, $entity, $hook_context);
    }
    return $links;
  }
  ...
  protected function buildLinks(CommentInterface $entity, EntityInterface $commented_entity) {
    ...
    if ($status == CommentItemInterface::OPEN) {
      ...
      if ($entity->access('create')) {
        $links['comment-reply'] = [
          'title' => t('Reply'),
          'url' => Url::fromRoute('external_comment.reply', [
      ...
      if (!$entity->isPublished() && $entity->access('approve')) {
        $links['comment-approve'] = [
          'title' => t('Approve'),
          'url' => Url::fromRoute('external_comment.approve', ['external_comment' => $entity->id()]),
        ];
      }
    ...
    return [
      '#theme' => 'links__external_comment__comment',
