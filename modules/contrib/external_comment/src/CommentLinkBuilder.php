...
class CommentLinkBuilder implements CommentLinkBuilderInterface {
   ...
   *   The entity manager service.
   */
  public function __construct(AccountInterface $current_user, CommentManagerInterface $external_comment_manager, ModuleHandlerInterface $module_handler, TranslationInterface $string_translation, EntityManagerInterface $entity_manager) {
    ...
    foreach ($fields as $field_name => $detail) {
      ...
      if ($commenting_status != CommentItemInterface::HIDDEN) {
        ...
        if ($view_mode == 'teaser') {
          ...
          if ($this->currentUser->hasPermission('access external comments')) {
            if (!empty($entity->get($field_name)->external_comment_count)) {
              $links['comment-comments'] = [
                'title' => $this->formatPlural($entity->get($field_name)->external_comment_count, '1 comment', '@count comments'),
              ...
              if ($this->moduleHandler->moduleExists('history')) {
                $links['comment-new-comments'] = [
                  ...
                  'attributes' => [
                    ...
                    'data-history-node-last-comment-timestamp' => $entity->get($field_name)->last_external_comment_timestamp,
          ...
          if ($commenting_status == CommentItemInterface::OPEN) {
            $external_comment_form_location = $field_definition->getSetting('form_location');
            if ($this->currentUser->hasPermission('post external comments')) {
              $links['comment-add'] = [
              ...
              if ($external_comment_form_location == CommentItemInterface::FORM_SEPARATE_PAGE) {
                $links['comment-add']['url'] = Url::fromRoute('external_comment.reply', [
          ...
          // is allowed to post external comments and if this entity is allowing new
          // comments.
          if ($commenting_status == CommentItemInterface::OPEN) {
            $external_comment_form_location = $field_definition->getSetting('form_location');
            if ($this->currentUser->hasPermission('post external comments')) {
              ...
              if ($external_comment_form_location == CommentItemInterface::FORM_SEPARATE_PAGE || (!empty($entity->get($field_name)->external_comment_count) && $this->currentUser->hasPermission('access external comments'))) {
                $links['comment-add'] = [
                  'title' => $this->t('Add new comment'),
                  'attributes' => ['title' => $this->t('Share your thoughts and opinions.')],
                  'fragment' => 'comment-form',
      ...
      if (!empty($links)) {
        $entity_links['external_comment__' . $field_name] = [
          '#theme' => 'links__entity__external_comment__' . $field_name,
          '#links' => $links,
          '#attributes' => ['class' => ['links', 'inline']],
        ];
        if ($view_mode == 'teaser' && $this->moduleHandler->moduleExists('history') && $this->currentUser->isAuthenticated()) {
          $entity_links['external_comment__' . $field_name]['#cache']['contexts'][] = 'user';
          $entity_links['external_comment__' . $field_name]['#attached']['library'][] = 'external_comment/drupal.node-new-comments-link';
          // Embed the metadata for the "X new comments" link (if any) on this
          // entity.
          $entity_links['external_comment__' . $field_name]['#attached']['drupalSettings']['history']['lastReadTimestamps'][$entity->id()] = (int) history_read($entity->id());
          $new_comments = $this->commentManager->getCountNewComments($entity);
          if ($new_comments > 0) {
            $page_number = $this->entityManager
              ->getStorage('external_comment')
              ->getNewCommentPageNumber($entity->{$field_name}->external_comment_count, $new_comments, $entity, $field_name);
            $query = $page_number ? ['page' => $page_number] : NULL;
            $value = [
              'new_external_comment_count' => (int) $new_comments,
              'first_new_external_comment_link' => $entity->url('canonical', [
                'query' => $query,
                'fragment' => 'new',
              ]),
            ];
            $parents = ['external_comment', 'newCommentsLinks', $entity->getEntityTypeId(), $field_name, $entity->id()];
            NestedArray::setValue($entity_links['external_comment__' . $field_name]['#attached']['drupalSettings'], $parents, $value);
          }
        }
      }
    }
