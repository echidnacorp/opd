...
class CommentStatistics implements CommentStatisticsInterface {
  ...
  public function read($entities, $entity_type, $accurate = TRUE) {
    ...
    $stats = $this->database->select('external_comment_entity_statistics', 'ces', $options)
      ->fields('ces')
  ...
  public function delete(EntityInterface $entity) {
    $this->database->delete('external_comment_entity_statistics')
  ...
  public function create(FieldableEntityInterface $entity, $fields) {
    $query = $this->database->insert('external_comment_entity_statistics')
      ->fields([
        'entity_id',
        'entity_type',
        'field_name',
    ...
    foreach ($fields as $field_name => $detail) {
      ...
      if ($entity instanceof EntityOwnerInterface) {
        $last_external_comment_uid = $entity->getOwnerId();
      }
      if (!isset($last_external_comment_uid)) {
        // Default to current user when entity does not implement
        // EntityOwnerInterface or author is not set.
        $last_external_comment_uid = $this->currentUser->id();
      }
      // Default to REQUEST_TIME when entity does not have a changed property.
      $last_external_comment_timestamp = REQUEST_TIME;
      // @todo Make comment statistics language aware and add some tests. See
      //   https://www.drupal.org/node/2318875
      if ($entity instanceof EntityChangedInterface) {
        $last_external_comment_timestamp = $entity->getChangedTimeAcrossTranslations();
      }
      $query->values([
        'entity_id' => $entity->id(),
        ...
        'last_external_comment_name' => NULL,
  ...
  public function getMaximumCount($entity_type) {
    return $this->database->query('SELECT MAX(external_comment_count) FROM {external_comment_entity_statistics} WHERE entity_type = :entity_type', [':entity_type' => $entity_type])->fetchField();
  }
  ...
  public function getRankingInfo() {
    return [
      'external_comments' => [
        ...
        'join' => [
          'type' => 'LEFT',
          'table' => 'external_comment_entity_statistics',
          'alias' => 'ces',
          // Default to comment field as this is the most common use case for
          // nodes.
          'on' => "ces.entity_id = CAST(i.sid as CHAR(255)) AND ces.entity_type = 'node' AND ces.field_name = 'external_comment'",
        ],
        // Inverse law that maps the highest view count on the site to 1 and 0
        // to 0. Note that the ROUND here is necessary for PostgreSQL and SQLite
        // in order to ensure that the :external_comment_scale argument is treated as
        // a numeric type, because the PostgreSQL PDO driver sometimes puts
        // values in as strings instead of numbers in complex expressions like
        // this.
        'score' => '2.0 - 2.0 / (1.0 + ces.external_comment_count * (ROUND(:external_comment_scale, 4)))',
        'arguments' => [':external_comment_scale' => \Drupal::state()->get('external_comment.node_external_comment_statistics_scale') ?: 0],
      ],
    ];
  }
  ...
  public function update(CommentInterface $comment) {
    ...
    // the {external_comment_entity_statistics} table.
    if (!$this->state->get('external_comment.maintain_entity_statistics')) {
      return;
    }
    ...
    if ($count > 0) {
      // Comments exist.
      $last_reply_query = $this->database->select('external_comment_field_data', 'c');
      $last_reply_query->join($external_comment_entity_table, 'ce', 'c.cid = %alias.entity_id AND %alias.deleted = 0');
      ...
      $this->database->merge('external_comment_entity_statistics')
        ->fields([
          'cid' => $last_reply->cid,
          'external_comment_count' => $count,
          'last_external_comment_timestamp' => $last_reply->changed,
          'last_external_comment_name' => $last_reply->uid ? '' : $last_reply->name,
          'last_external_comment_uid' => $last_reply->uid,
        ])
        ->keys([
      ...
      $this->database->update('external_comment_entity_statistics')
        ->fields([
          'cid' => 0,
          'external_comment_count' => 0,
          // Use the changed date of the entity if it's set, or default to
          // REQUEST_TIME.
          'last_external_comment_timestamp' => ($entity instanceof EntityChangedInterface) ? $entity->getChangedTimeAcrossTranslations() : REQUEST_TIME,
          'last_external_comment_name' => '',
          'last_external_comment_uid' => $last_external_comment_uid,
        ])
