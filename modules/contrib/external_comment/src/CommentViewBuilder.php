...
class CommentViewBuilder extends EntityViewBuilder {
  ...
  protected function getBuildDefaults(EntityInterface $entity, $view_mode) {
    ...
    $build['#external_comment_threaded'] = $entity->getCommentedEntity()
      ->getFieldDefinition($entity->getFieldName())
      ->getSetting('default_mode') === CommentManagerInterface::EXTERNAL_COMMENT_MODE_THREADED;
    // If threading is enabled, don't render cache individual comments, but do
    // keep the cacheability metadata, so it can bubble up.
    if ($build['#external_comment_threaded']) {
  ...
  public function buildComponents(array &$build, array $entities, array $displays, $view_mode) {
    /** @var \Drupal\external_comment\CommentInterface[] $entities */
    if (empty($entities)) {
      return;
    }
    ...
    foreach ($entities as $id => $entity) {
      if ($build[$id]['#external_comment_threaded']) {
        $external_comment_indent = count(explode('.', $entity->getThread())) - 1;
        if ($external_comment_indent > $current_indent) {
          ...
          // depth is higher.
          $build[$id]['#external_comment_indent'] = 1;
          $current_indent++;
        }
        else {
          // Set zero if this comment is on the same level as the previous one
          // or negative value to point an amount indents to close.
          $build[$id]['#external_comment_indent'] = $external_comment_indent - $current_indent;
          $current_indent = $external_comment_indent;
        }
      }
      ...
      $build[$id]['#theme'] = 'external_comment__' . $entity->getFieldName() . '__' . $commented_entity->bundle();

      $display = $displays[$entity->bundle()];
      if ($display->getComponent('links')) {
        $build[$id]['links'] = [
          '#lazy_builder' => ['external_comment.lazy_builders:renderLinks', [
            $entity->id(),
            $view_mode,
      ...
      $build[$id]['#attached']['library'][] = 'external_comment/drupal.comment-by-viewer';
      if ($this->moduleHandler->moduleExists('history') && $this->currentUser->isAuthenticated()) {
        $build[$id]['#attached']['library'][] = 'external_comment/drupal.comment-new-indicator';
    ...
    if ($build[$id]['#external_comment_threaded']) {
      ...
      $build[$id]['#external_comment_indent_final'] = $current_indent;
    }
  }
  ...
  protected function alterBuild(array &$build, EntityInterface $comment, EntityViewDisplayInterface $display, $view_mode) {
    ...
    if (empty($comment->in_preview)) {
      ...
      if ($build['#external_comment_threaded']) {
        $prefix .= $build['#external_comment_indent'] <= 0 ? str_repeat('</div>', abs($build['#external_comment_indent'])) : "\n" . '<div class="indented">';
      }
      ...
      // Close all open divs.
      if (!empty($build['#external_comment_indent_final'])) {
        $build['#suffix'] = str_repeat('</div>', $build['#external_comment_indent_final']);
      }
    }
  }
