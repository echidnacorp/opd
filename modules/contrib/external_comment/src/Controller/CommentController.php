...
class CommentController extends ControllerBase {
   ...
   */
  public function __construct(HttpKernelInterface $http_kernel, CommentManagerInterface $external_comment_manager, EntityManagerInterface $entity_manager) {
  ...
  public function redirectNode(EntityInterface $node) {
    ...
    if (!empty($fields) && ($field_names = array_keys($fields)) && ($field_name = reset($field_names))) {
      return $this->redirect('external_comment.reply', [
        'entity_type' => 'node',
        'entity' => $node->id(),
   ...
   *     - external_comment_entity: If the comment is a reply to the entity.
   *     - external_comment_parent: If the comment is a reply to another comment.
   *   - external_comment_form: The comment form as a renderable array.
   *
   * @throws \Symfony\Component\HttpKernel\Exception\NotFoundHttpException
   */
  public function getReplyForm(Request $request, EntityInterface $entity, $field_name, $pid = NULL) {
    ...
    if ($request->request->get('op') != $this->t('Preview')) {
      ...
      if ($pid) {
        ...
        $comment = $this->entityManager()->getStorage('external_comment')->load($pid);
        // Display the parent comment.
        $build['external_comment_parent'] = $this->entityManager()->getViewBuilder('external_comment')->view($comment);
      }
    ...
    $comment = $this->entityManager()->getStorage('external_comment')->create([
      'commented_' . $entity->getEntityTypeId() => $entity->id(),
      'pid' => $pid,
      'entity_type' => $entity->getEntityTypeId(),
      'field_name' => $field_name,
    ]);
    $build['external_comment_form'] = $this->entityFormBuilder()->getForm($comment);
  ...
  public function replyFormAccess(EntityInterface $entity, $field_name, $pid = NULL) {
    ...
    $access = AccessResult::allowedIfHasPermission($account, 'post external comments');
    ...
    if ($pid) {
      // Check if the user has the proper permissions.
      $access = $access->andIf(AccessResult::allowedIfHasPermission($account, 'access external comments'));
  ...
  public function renderNewCommentsNodeLinks(Request $request) {
    ...
    foreach ($nids as $nid) {
      ...
      $page_number = $this->entityManager()->getStorage('external_comment')
        ->getNewCommentPageNumber($node->{$field_name}->external_comment_count, $new, $node, $field_name);
      $query = $page_number ? ['page' => $page_number] : NULL;
      $links[$nid] = [
        'new_external_comment_count' => (int) $new,
        'first_new_external_comment_link' => $this->getUrlGenerator()->generateFromRoute('entity.node.canonical', ['node' => $node->id()], ['query' => $query, 'fragment' => 'new']),
      ];
    }
