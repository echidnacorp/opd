 ...
 *   label_singular = @Translation("external comment"),
 *   label_plural = @Translation("external comments"),
 *   label_count = @PluralTranslation(
 *     singular = "@count external comment",
 *     plural = "@count external comments",
 *   ),
 *   bundle_label = @Translation("External comment type"),
 *   handlers = {
 *     "storage" = "Drupal\external_comment\CommentStorage",
 *     "storage_schema" = "Drupal\external_comment\CommentStorageSchema",
 *     "access" = "Drupal\external_comment\CommentAccessControlHandler",
 *     "list_builder" = "Drupal\Core\Entity\EntityListBuilder",
 *     "view_builder" = "Drupal\external_comment\CommentViewBuilder",
 *     "views_data" = "Drupal\external_comment\CommentViewsData",
 *     "form" = {
 *       "default" = "Drupal\external_comment\CommentForm",
 *       "delete" = "Drupal\external_comment\Form\DeleteForm"
 *     },
 *     "translation" = "Drupal\external_comment\CommentTranslationHandler"
 *   },
 *   base_table = "external_comment",
 *   data_table = "external_comment_field_data",
 *   uri_callback = "external_comment_uri",
 *   translatable = TRUE,
 *   entity_keys = {
 *     "id" = "cid",
 *     "bundle" = "external_comment_type",
 *     "label" = "subject",
 *     "langcode" = "langcode",
 *     "uuid" = "uuid",
 *     "published" = "status",
 *   },
 *   links = {
 *     "canonical" = "/external/comment/{comment}",
 *     "delete-form" = "/external/comment/{comment}/delete",
 *     "edit-form" = "/external/comment/{comment}/edit",
 *   },
 *   bundle_entity_type = "external_comment_type",
 *   field_ui_base_route  = "entity.external_comment_type.edit_form",
 *   constraints = {
 *     "CommentName" = {}
 *   }
 * )
...
class Comment extends ContentEntityBase implements CommentInterface {
  ...
  public function preSave(EntityStorageInterface $storage) {
    ...
    if (is_null($this->get('status')->value)) {
      if (\Drupal::currentUser()->hasPermission('skip external comment approval')) {
        $this->setPublished();
      }
      else {
        $this->setUnpublished();
    ...
    if ($this->isNew()) {
      ...
      // @see \Drupal\external_comment\CommentViewBuilder::buildComponents()
  ...
  public function postSave(EntityStorageInterface $storage, $update = TRUE) {
    ...
    // Update the {external_comment_entity_statistics} table prior to executing the hook.
    \Drupal::service('external_comment.statistics')->update($this);
  }
  ...
  public static function postDelete(EntityStorageInterface $storage, array $entities) {
    ...
    entity_delete_multiple('external_comment', $child_cids);

    foreach ($entities as $id => $entity) {
      \Drupal::service('external_comment.statistics')->update($entity);
    }
  }
  ...
  public static function baseFieldDefinitions(EntityTypeInterface $entity_type) {
    ...
    $fields['external_comment_type']->setLabel(t('Comment Type'))
    ...
    $fields['pid'] = BaseFieldDefinition::create('entity_reference')
      ...
      ->setSetting('target_type', 'external_comment');
  ...
  public static function preCreate(EntityStorageInterface $storage, array &$values) {
    if (empty($values['external_comment_type']) && !empty($values['field_name']) && !empty($values['entity_type'])) {
      $field_storage = FieldStorageConfig::loadByName($values['entity_type'], $values['field_name']);
      $values['external_comment_type'] = $field_storage->getSetting('external_comment_type');
    }
  }
