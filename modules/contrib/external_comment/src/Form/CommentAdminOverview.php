...
class CommentAdminOverview extends FormBase {
   ...
   *   The module handler.
   */
  public function __construct(EntityManagerInterface $entity_manager, CommentStorageInterface $external_comment_storage, DateFormatterInterface $date_formatter, ModuleHandlerInterface $module_handler) {
  ...
  public function getFormId() {
    return 'external_comment_admin_overview';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $type = 'new') {
    ...
    /** @var $comments \Drupal\external_comment\CommentInterface[] */
    ...
    foreach ($comments as $comment) {
      ...
      $external_comment_permalink = $comment->permalink();
      if ($comment->hasField('comment_body') && ($body = $comment->get('comment_body')->value)) {
        $attributes = $external_comment_permalink->getOption('attributes') ?: [];
        $attributes += ['title' => Unicode::truncate($body, 128)];
        $external_comment_permalink->setOption('attributes', $attributes);
      }
      $options[$comment->id()] = [
        ...
        'subject' => [
          'data' => [
            ...
            '#url' => $external_comment_permalink,
          ],
        ],
        'author' => [
          'data' => [
      ...
      $external_comment_uri_options = $comment->urlInfo()->getOptions() + ['query' => $destination];
      $links = [];
      $links['edit'] = [
        'title' => $this->t('Edit'),
        'url' => $comment->urlInfo('edit-form', $external_comment_uri_options),
      ];
      if ($this->moduleHandler->moduleExists('content_translation') && $this->moduleHandler->invoke('content_translation', 'translate_access', [$comment])->isAllowed()) {
        $links['translate'] = [
          ...
          'url' => $comment->urlInfo('drupal:content-translation-overview', $external_comment_uri_options),
        ];
      }
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    $form_state->setValue('external_comments', array_diff($form_state->getValue('external_comments'), [0]));
    // We can't execute any 'Update options' if no comments were selected.
    if (count($form_state->getValue('external_comments')) == 0) {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $cids = $form_state->getValue('external_comments');

    foreach ($cids as $cid) {
      // Delete operation handled in \Drupal\external_comment\Form\ConfirmDeleteMultiple
      // see \Drupal\external_comment\Controller\AdminController::adminPage().
