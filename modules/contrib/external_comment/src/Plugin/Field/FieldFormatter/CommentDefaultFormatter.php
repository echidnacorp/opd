...
class CommentDefaultFormatter extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function __construct($plugin_id, $plugin_definition, FieldDefinitionInterface $field_definition, array $settings, $label, $view_mode, array $third_party_settings, AccountInterface $current_user, EntityManagerInterface $entity_manager, EntityFormBuilderInterface $entity_form_builder, RouteMatchInterface $route_match) {
    ...
    $this->viewBuilder = $entity_manager->getViewBuilder('external_comment');
    $this->storage = $entity_manager->getStorage('external_comment');
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    if ($status != CommentItemInterface::HIDDEN && empty($entity->in_preview) &&
      ...
      // external_comment_node_update_index() instead of by this formatter, so don't
      // return anything if the view mode is search_index or search_result.
      !in_array($this->viewMode, ['search_result', 'search_index'])) {
      $external_comment_settings = $this->getFieldSettings();

      // Only attempt to render comments if the entity has visible comments.
      // Unpublished comments are not included in
      // $entity->get($field_name)->external_comment_count, but unpublished comments
      // should display if the user is an administrator.
      $elements['#cache']['contexts'][] = 'user.permissions';
      if ($this->currentUser->hasPermission('access external comments') || $this->currentUser->hasPermission('administer external comments')) {
        $output['external_comments'] = [];

        if ($entity->get($field_name)->external_comment_count || $this->currentUser->hasPermission('administer external comments')) {
          $mode = $external_comment_settings['default_mode'];
          $comments_per_page = $external_comment_settings['per_page'];
          ...
          if ($comments) {
            ...
            $output['external_comments'] += $build;
          }
        }
      }
      ...
      if ($status == CommentItemInterface::OPEN && $external_comment_settings['form_location'] == CommentItemInterface::FORM_BELOW && $this->viewMode != 'print') {
        // Only show the add comment form if the user has permission.
        $elements['#cache']['contexts'][] = 'user.roles';
        if ($this->currentUser->hasPermission('post external comments')) {
          $output['external_comment_form'] = [
            '#lazy_builder' => ['external_comment.lazy_builders:renderForm', [
              $entity->getEntityTypeId(),
              $entity->id(),
              $field_name,
              $this->getFieldSetting('external_comment_type'),
            ]],
      ...
      $elements[] = $output + [
        '#external_comment_type' => $this->getFieldSetting('external_comment_type'),
        '#external_comment_display_mode' => $this->getFieldSetting('default_mode'),
        'external_comments' => [],
  ...
  public function calculateDependencies() {
    ...
    if ($mode = $this->getSetting('view_mode')) {
      if ($bundle = $this->getFieldSetting('external_comment_type')) {
  ...
  protected function getViewModes() {
    return $this->entityManager->getViewModeOptionsByBundle('external_comment', $this->getFieldSetting('external_comment_type'));
  }
