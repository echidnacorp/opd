...
class NodeNewComments extends NumericField {
  ...
  public function init(ViewExecutable $view, DisplayPluginBase $display, array &$options = NULL) {
    ...
    $this->additional_fields['external_comment_count'] = ['table' => 'external_comment_entity_statistics', 'field' => 'external_comment_count'];
  }
  ...
  public function preRender(&$values) {
    ...
    if ($nids) {
      ...
      $result = $this->database->query("SELECT n.nid, COUNT(c.cid) as num_comments FROM {node} n
        INNER JOIN {" . $external_comment_entity_table . "} ce ON c.cid = ce.entity_id AND ce.deleted = 0
        INNER JOIN {external_comment_field_data} c ON n.nid = c.$target_id_column AND c.entity_type = 'node' AND c.default_langcode = 1
  ...
  protected function renderLink($data, ResultRow $values) {
    if (!empty($this->options['link_to_comment']) && $data !== NULL && $data !== '') {
      ...
      $field_map = $entity_manager->getFieldMapByFieldType('external_comment');
      $external_comment_field_name = 'external_comment';
      foreach ($field_map['node'] as $field_name => $field_data) {
        foreach ($field_data['bundles'] as $bundle_name) {
          if ($node_type == $bundle_name) {
            $external_comment_field_name = $field_name;
            break 2;
          }
        }
      }
      $page_number = $entity_manager->getStorage('external_comment')
        ->getNewCommentPageNumber($this->getValue($values, 'external_comment_count'), $this->getValue($values), $node, $external_comment_field_name);
