...
class InlineEntityFormComplex extends InlineEntityFormBase implements ContainerFactoryPluginInterface {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $element['empty_mode'] = [
      '#type' => 'select',
      '#title' => $this->t('Default form when the field is empty'),
      '#default_value' => $this->getSetting('empty_mode'),
      '#options' => $this->getEmptyModeOptions(),
      '#states' => [
        'visible' => [
          ':input[name="' . $states_prefix . '[allow_existing]"]' => ['checked' => TRUE],
          ':input[name="' . $states_prefix . '[allow_new]"]' => ['checked' => TRUE],
        ],
      ],
    ];
  ...
  public function settingsSummary() {
    ...
    switch ($this->getSetting('empty_mode')) {
      case 'new':
        $summary[] = $this->t('For empty fields, the form to add new @label is shown.', ['@label' => $labels['plural']]);
        break;
      case 'existing':
        $summary[] = $this->t('For empty fields, the form to add existing @label is shown.', ['@label' => $labels['plural']]);
        break;
    }
  ...
  /**
   * Returns the options for the empty mode.
   *
   * @return array
   *   List of options.
   */
  protected function getEmptyModeOptions() {
    $labels = $this->getEntityTypeLabels();
    return [
      '' => $this->t('Show buttons to chose'),
      'new' => $this->t('Create new @label', ['@label' => $labels['singular']]),
      'existing' => $this->t('Add existing @label', ['@label' => $labels['singular']]),
    ];
  }
  /**
   * {@inheritdoc}
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    // When no entities are present, determine what to do.
    // If the field is required or if a default mode is configured, try to open
    // one of the two forms.
    if (empty($entities) && ($this->fieldDefinition->isRequired() || !empty($settings['empty_mode']))) {
      // Check if the mode switching button was clicked and show the requested form.
      $requested_form = $form_state->get(['inline_entity_form', $this->getIefId(), 'form']);
      if (!empty($requested_form)) {
        $switch_mode = $hide_cancel = $requested_form == 'add' ? $settings['allow_existing'] : $allow_new;
      }
      // Or show the form to add existing entities.
      else if ($settings['allow_existing'] && (!$allow_new || $settings['empty_mode'] == 'existing')) {
        $form_state->set(['inline_entity_form', $this->getIefId(), 'form'], 'ief_add_existing');
        $hide_cancel = TRUE;
        $switch_mode = $allow_new;
      }
      // Or show the form to add new entities.
      elseif ($create_bundles_count == 1 && $allow_new && (!$settings['allow_existing'] || $settings['empty_mode'] == 'new')) {
        ...
        if ($parent_entity_type != $target_type || $parent_bundle != $bundle) {
          ...
          $hide_cancel = TRUE;
          $switch_mode = $settings['allow_existing'];
        }
      }
    }
    ...
    if (empty($open_form)) {
      ...
      if ($hide_cancel || $switch_mode) {
        ...
        if ($switch_mode) {
          $process_element['#process'][] = [get_class($this), 'switchMode'];
        }
      }
   ...
   *   Form array structure.
   */
  public static function hideCancel($element, FormStateInterface $form_state) {
  ...
  /**
   * Shows a switch mode link.
   *
   * @param array $element
   *   Form array structure.
   */
  public static function switchMode($element, FormStateInterface $form_state) {
    $element['actions']['ief_switch_mode'] = [
      '#type' => 'submit',
      '#name' => 'ief-switch-mode-' . $element['#ief_id'],
      '#limit_validation_errors' => [],
      '#ajax' => [
        'callback' => 'inline_entity_form_get_element',
        'wrapper' => 'inline-entity-form-' . $element['#ief_id'],
    ...
    if (isset($element['actions']['ief_add_cancel'])) {
      $element['actions']['ief_switch_mode']['#value'] =  t('Add existing @type_singular instead', ['@type_singular' => $element['#ief_labels']['singular']]);;
      $element['actions']['ief_switch_mode']['#ief_form'] = 'ief_add_existing';
    }
    elseif (isset($element['actions']['ief_reference_cancel'])) {
      $element['actions']['ief_switch_mode']['#value'] =  t('Create new @type_singular instead', ['@type_singular' => $element['#ief_labels']['singular']]);;
      $element['actions']['ief_switch_mode']['#ief_form'] = 'add';
      if (empty($element['#required'])) {
        $element['entity_id']['#required'] = FALSE;
      }
    }
    return $element;
  }
