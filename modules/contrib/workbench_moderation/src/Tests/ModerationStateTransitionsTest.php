...
class ModerationStateTransitionsTest extends ModerationStateTestBase {
  ...
  public function testTransitionAdministration() {
    ...
    // Verify that Labels are identical in case where from and to states
    // are different.
    $published_expired_transition = ModerationStateTransition::load('draft_needs_review');
    $this->drupalGet('admin/structure/workbench-moderation/transitions/draft_needs_review');
    $this->assertFieldByName('label', $published_expired_transition->label());
    $this->assertFieldByName('label', $published_expired_transition->label(FALSE));

    // Add self-transition and verify the label.
    $this->drupalGet('admin/structure/workbench-moderation/transitions');
    $this->clickLink(t('Add Moderation state transition'));
    $this->drupalPostForm(NULL, [
      'label' => 'Draft » Draft',
    ...
    // Verify that Labels are different for self-transitions.
    $draft_draft_new_transition = ModerationStateTransition::load('draft_draft_new');
    $this->drupalGet('admin/structure/workbench-moderation/transitions/draft_draft_new');
    $this->assertFieldByName('label', $draft_draft_new_transition->label());
    $this->assertNoFieldByName('label', $draft_draft_new_transition->label(FALSE));
  }
