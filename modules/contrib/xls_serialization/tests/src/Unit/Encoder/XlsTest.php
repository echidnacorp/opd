...
class XlsTest extends UnitTestCase {
  ...
  public function testEncode() {
    ...
    $sheet = $file->getSheet();
    // Verify headers.
    $this->assertEquals('foo', $sheet->getCellByColumnAndRow(0, 1)->getValue());
    $this->assertEquals('biz', $sheet->getCellByColumnAndRow(1, 1)->getValue());

    // Verify some of the data.
    $this->assertEquals('bar1', $sheet->getCellByColumnAndRow(0, 3)
      ->getValue());
    $this->assertEquals('baz2', $sheet->getCellByColumnAndRow(1, 4)
      ->getValue());
  }
  ...
  protected function loadXlsFile($xls, $format = 'xls') {
    ...
    $xls_file = @tempnam(\PHPExcel_Shared_File::sys_get_temp_dir(), 'phpxltmp.' . $format);
    file_put_contents($xls_file, $xls);
    return \PHPExcel_IOFactory::load($xls_file);
  }
